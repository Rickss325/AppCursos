        ...
        MDBoxLayout:
            ...
            MDBoxLayout:
                ...
                MDCard:
                    ...
                    padding: '22dp'
                    radius: [18]
                    md_bg_color: root.card_color_r, root.card_color_g, root.card_color_b, 0.15
                    elevation: 0
...
class MainApp(MDApp):
    ...
    def _show_result_screen(self):
        ...
        screen.result_emoji    = '🏆' if pct >= 90 else '🎉'
    ...
    def generate_certificate(self):
        ...
        c.setStrokeColor(HexColor('#9A9AB0'))
        c.setLineWidth(1)
        c.line(100, 90, 280, 90)
        c.setFont('Helvetica', 10)
        c.setFillColor(HexColor('#9A9AB0'))
        c.drawCentredString(190, 76, 'Director Académico')
        c.setFont('Helvetica-Bold', 11)
        c.setFillColor(white)
        c.drawCentredString(190, 60, 'EduCursos')

        # Sello central
        c.setStrokeColor(HexColor('#34D9A4'))
        c.setLineWidth(2)
        c.circle(W / 2, 85, 38, fill=0, stroke=1)
        c.setStrokeColor(HexColor('#7357F2'))
        c.setLineWidth(1)
        c.circle(W / 2, 85, 32, fill=0, stroke=1)
        c.setFont('Helvetica-Bold', 9)
        c.setFillColor(HexColor('#34D9A4'))
        c.drawCentredString(W / 2, 90, 'CERTIFICADO')
        c.drawCentredString(W / 2, 78, 'OFICIAL')

        # Línea firma derecha
        c.setStrokeColor(HexColor('#9A9AB0'))
        c.setLineWidth(1)
        c.line(W - 280, 90, W - 100, 90)
        c.setFont('Helvetica', 10)
        c.setFillColor(HexColor('#9A9AB0'))
        c.drawCentredString(W - 190, 76, 'Coordinador del Curso')
        c.setFont('Helvetica-Bold', 11)
        c.setFillColor(white)
        c.drawCentredString(W - 190, 60, self.current_curso['titulo'][:28])
